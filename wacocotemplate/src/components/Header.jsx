      ...
      <AppBar component="nav" style={{ background: '#000639' }}>
        <Toolbar>
          <Typography
            variant="h6"
            component="div"
            sx={{ flexGrow: 1,  ml: 5, display: { xs: 'none', sm: 'block',textAlign: 'left'   } }}
          >
          ...
          <Box sx={{  display: { xs: 'none', sm: 'block' } }}>
            {navItems.map((item) => (
              <Button  key={item} sx={{ color: '#fff',textTransform: 'capitalize' }}>
                <Link style={{textDecoration:"none",color:"#ffffff" }} to={`/${item.split(' ').join('')}`}>
                  {item}
                </Link>
              </Button>
            ))}
          </Box>
          <Button variant="contained" sx={{background: '#f94363'}}>Contact us</Button>
        </Toolbar>
      </AppBar>
    </>
  )
